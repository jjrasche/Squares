var yesterdaysGamesNotFinishedQuery = [
  {'finished': {$eq: false}},
  {'date': {
...
Meteor.methods({
  'changeDataRefreshCriteria' : function changeScrapeRate(rate, date) {
    if (Meteor.isServer) {
      if (rate) {
      ...
      if (date) {
        Meteor.call(scrapeGameData(date))
      }
    }
  },
  'scrapeGameData' : function scrapeGameData(date) {
    if (Meteor.isServer) {
      ...
      if (Game.find({$and: yesterdaysGamesNotFinishedQuery}).length) 
        startDate.addDays(-1);
...
insertOrUpdateGame = function insertOrUpdateGame(game) {
  ...
  if (exists) {
    ...
    console.log("updated: ", exists._id, game.homeScore + ' ' + game.awayScore + ' ' + game.date + ' ' + game.finished);
  }
  else {
