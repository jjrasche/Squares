...
Meteor.methods({
  'changeScrapeRate' : function changeScrapeRate(seconds) {
    if (Meteor.isServer) {
  ...
  'scrapeGameData' : function scrape(args) {
    if (Meteor.isServer) {
      ...
      var startDate = new Date()//"20150319".parseYYYYmmddDate();
      var endDate = new Date()//"20150319".parseYYYYmmddDate();
      var games = [];
...
insertOrUpdateGame = function insertOrUpdateGame(game) {
  ...
  if (exists) {
    ...
    console.log("updated: ", exists._id, game.homeScore + ' ' + game.awayScore + ' ' + game.finished);
  }
  else {
