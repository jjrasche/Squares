...
formatBoardData = function formatBoardData() {
	var board = getBoard();
	var games = getGames(board,{});
	var matrix = getGamesMatrix(games);
	var boardArray = []
	...
	for (var x = 0; x < 10; x++) {
		for (var y = 0; y < 10; y++) {
			...
			if ((squareSelected(x, y) && Session.get('boardPageEditMode')) ||
				squareContainsSelectedGame(matrix[x][y]))
				color = 'blue';
...
getGamesMatrix = function getGamesMatrix(games) {
	var matrix = createEmptyMatrix();
	for (var i = 0; i < games.length; i++) {
		var game = games[i];
		var losingScore = Math.min(game.awayScore, game.homeScore);
		var winningScore = Math.max(game.awayScore, game.homeScore);
		matrix[winningScore%10][losingScore%10].push(game);
	}
	return matrix;
}
...
getGames = function getGames(board, query) {
	return Game.find(query).fetch();
}
